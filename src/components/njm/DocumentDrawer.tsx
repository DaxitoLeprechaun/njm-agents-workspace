...
export function DocumentDrawer({ open, onClose, document: doc }: DocumentDrawerProps) {
  ...
  return (
    <>
      ...
      {/* Drawer */}
      <div className="fixed right-0 top-0 z-50 flex h-full w-full max-w-2xl flex-col border-l border-border bg-surface-1 shadow-2xl animate-slide-in-right">
        {/* Header */}
        <div className="flex items-center justify-between border-b border-border px-6 py-4">
          ...
          <button onClick={onClose} className="rounded-lg p-2 text-muted-foreground transition-colors hover:bg-surface-2 hover:text-foreground">
            <X className="h-5 w-5" />
          </button>
        </div>
        ...
        <div className="flex-1 overflow-auto p-6 scrollbar-thin">
          {/* Suggestion card */}
          <div className="mb-6 rounded-xl border border-pm/30 bg-pm/5 p-4">
            <div className="flex items-start gap-3">
              <div className="flex h-8 w-8 shrink-0 items-center justify-center rounded-lg bg-pm/20">
          ...
          {/* Editor */}
          <div className="rounded-xl bg-surface-0 p-6">
        ...
        <div className="flex items-center justify-end gap-3 border-t border-border px-6 py-4">
          <button className="rounded-lg border border-border px-4 py-2 text-sm font-medium text-muted-foreground transition-colors hover:bg-surface-2 hover:text-foreground">
            Proponer Ajustes
          </button>
          <button className="rounded-lg bg-pm px-4 py-2 text-sm font-medium text-foreground transition-colors hover:bg-pm/80">
            Aprobar y Guardar
          </button>
        </div>
      </div>
